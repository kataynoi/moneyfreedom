...
class Admin_account extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('Admin_account_model', 'crud');
    }
    ...
    public function index()
    {
        ...
        $this->layout->view('admin_account/index', $data);
    }


    function fetch_admin_account()
    {
        ...
        foreach ($fetch_data as $row) {
            ...
            $sub_array = array();
                $sub_array[] = $row->id;$sub_array[] = $row->name;$sub_array[] = $row->account_number;$sub_array[] = $row->bank;$sub_array[] = $row->active;
    ...
    public function del_admin_account(){
        $id = $this->input->post('id');

        $rs=$this->crud->del_admin_account($id);
        if($rs){
            $json = '{"success": true}';
        }else{
            $json = '{"success": false}';
    ...
    public function  save_admin_account()
    {
            ...
            if($data['action']=='insert'){
                $rs=$this->crud->save_admin_account($data);
                if($rs){
                ...
                $rs=$this->crud->update_admin_account($data);
                    if($rs){
                        $json = '{"success": true}';
                    }else{
                        $json = '{"success": false}';
    ...
    {
                $id = $this->input->post('id');
                $rs = $this->crud->get_admin_account($id);
                $rows = json_encode($rs);
