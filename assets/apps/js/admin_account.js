$(document).ready(function () {
    var dataTable = $('#table_data').DataTable({
        ...
        "pageLength": 50,
        "ajax": {
            url: site_url + '/admin_account/fetch_admin_account',
            data: {
...
crud.ajax = {
    del_data: function (id, cb) {
        var url = '/admin_account/del_admin_account',
            params = {
                id: id
            }
        ...
        var url = '/admin_account/save_admin_account',
            params = {
                items: items
            }
        ...
        var url = '/admin_account/get_admin_account',
            params = {
                id: id
            }
...
crud.set_after_update = function (items, row_id) {
    ...
    row_id.find("td:eq(2)").html(items.account_number);
    row_id.find("td:eq(3)").html(items.bank);
    row_id.find("td:eq(4)").html(items.active);

}
crud.set_after_insert = function (items, id) {

    $('<tr name="row' + (id + 1) + '"><td>' + id + '</td>' +
        '<td>' + items.id + '</td>' + '<td>' + items.name + '</td>' + '<td>' + items.account_number + '</td>' + '<td>' + items.bank + '</td>' + '<td>' + items.active + '</td>' +
...
crud.set_update = function (data, row_id) {
    ...
    $("#account_number").val(data.rows["account_number"]);
    $("#bank").val(data.rows["bank"]);
...
$('#btn_save').on('click', function (e) {
    ...
    items.id = $("#id").val();
    items.name = $("#name").val();
    items.account_number = $("#account_number").val();
    items.bank = $("#bank").val();
...
function validate(items) {
     ...
     if (!items.name) {
        swal("กรุณาระบุชื่อบัญชี");
        $("#name").focus();
    } else if (!items.account_number) {
        swal("กรุณาระบุเลขที่บัญชี");
        $("#account_number").focus();
    } else if (!items.bank) {
        swal("กรุณาระบุธนาคาร");
        $("#bank").focus();
